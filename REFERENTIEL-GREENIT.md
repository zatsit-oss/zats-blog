...
Évaluation du référentiel [cnumr/best-practices](https://github.com/cnumr/best-practices) appliquée à ce dépôt, le 29 août 2026.
...
| Statut | Nombre | Part des applicables |
|---|---:|---:|
| ✅ Respectée | 66 | 78 % |
| 🟡 Partielle | 18 | 21 % |
| ❌ Non respectée | 1 | 1 % |
| ⬜ Non applicable | 34 | hors périmètre |
| **Total** | **119** | |

**66 règles respectées sur 85 applicables**, soit 78 %. Une seule est franchement non respectée, et dix-huit le sont partiellement.
...
La majorité des règles sont respectées non par optimisation mais par choix d'architecture : site statique, aucun CMS, aucune base de données, aucun traceur, aucun cookie, aucun framework côté client. Trente-quatre règles sont sans objet pour cette raison, et ce n'est pas une facilité : c'est le résultat de la décision de quitter Docusaurus, qui envoyait 135 ko de JavaScript compressé là où ce site en envoie 1,1.

### Chiffres du build au 29 août 2026

| Mesure | Valeur | Budget interne |
|---|---:|---:|
| Poids initial, texte gzippé | 63 à 81 ko | < 500 ko |
| Poids total, pire page | 205 ko | < 1 Mo |
| Requêtes initiales | 9 à 11 | < 25 |
| Éléments du DOM, pire page | 483 | < 1500 |
| JavaScript, tout le site | 4 ko non compressé | argumenté ligne à ligne |
| Feuilles de style | 5 | |
| Fichiers de police | 6, soit 48,5 ko | 3 graisses au plus |
| Pages construites | 68 | |

---

## Spécification

*5 règles : 4 respectées, 1 non applicable.*
...
| `004` | Préférer la saisie assistée à l'autocomplétion | ⬜ Non applicable | Aucun formulaire de saisie sur le site. Le seul champ est la recherche, qui interroge un index local. |
...
*23 règles : 15 respectées, 3 partielles, 5 non applicables.*
...
| `008` | Proposer un traitement asynchrone lorsque c'est possible | ⬜ Non applicable | Aucun traitement long côté client ni serveur : le site est entièrement généré à la publication. |
...
| `073` | Ne se connecter à une base de données que si nécessaire | ⬜ Non applicable | Aucune base de données. |
...
| `110` | N'utiliser que des fichiers double opt-in | ⬜ Non applicable | Aucune liste de diffusion. |
...
| `4019` | Préférer une PWA à une application mobile native similaire au site web | ⬜ Non applicable | Aucune application mobile : le site est une page web. |
| `4021` | Mettre en place une architecture élastique | ⬜ Non applicable | Site statique : pas d'architecture à dimensionner. |
...
*43 règles : 29 respectées, 5 partielles, 1 non respectée, 8 non applicables.*
...
| `016` | Utiliser certains forks applicatifs orientés "performance" | ⬜ Non applicable | Aucun fork applicatif : Astro et Sätteri sont utilisés dans leur version publiée. |
...
| `038` | Utiliser le rechargement partiel d'une zone de contenu | ⬜ Non applicable | Pas de rechargement partiel : les pages sont statiques et le navigateur les met en cache. |
...
| `042` | Rendre les éléments du DOM invisibles lors de leur modification | ⬜ Non applicable | Aucune modification massive du DOM : les pages sont rendues au build. |
...
| `072` | Éviter d'effectuer des requêtes SQL à l’intérieur d’une boucle | ⬜ Non applicable | Aucune base de données. |
| `075` | Optimiser les requêtes aux bases de données | ⬜ Non applicable | Aucune base de données. |
...
| `4007` | Économiser de la bande passante grâce à un Service Worker | ⬜ Non applicable | Aucun service worker, et c'est cohérent : un site statique avec un bon cache HTTP n'a pas besoin d'un script pour rejouer ce que le navigateur fait déjà. |
...
| `4037` | Bien choisir son thème et limiter le nombre d'extensions dans un CMS | ⬜ Non applicable | Aucun CMS, aucune extension. |
| `4038` | Sécuriser l'accès à l'administration | ⬜ Non applicable | Aucune interface d'administration : la publication passe par une revue de pull request. |
...
*26 règles : 10 respectées, 5 partielles, 11 non applicables.*
...
| `057` | Utiliser tous les niveaux de cache du CMS | ⬜ Non applicable | Aucun CMS. |
...
| `087` | Privilégier un fournisseur d'électricité écoresponsable | ⬜ Non applicable | Le choix du fournisseur d'électricité ne relève pas de ce dépôt. |
...
| `089` | Utiliser des serveurs virtualisés | ⬜ Non applicable | Aucun serveur : le site est servi depuis un stockage d'objets. |
| `090` | Optimiser l'efficacité énergétique des serveurs | ⬜ Non applicable | Ne relève pas de ce dépôt. |
...
| `092` | Mettre les caches entièrement en RAM (opcode et kvs) | ⬜ Non applicable | Aucun cache applicatif : pas d'application. |
...
| `097` | Utiliser un serveur asynchrone | ⬜ Non applicable | Aucun serveur applicatif. |
...
| `102` | Mettre en cache les réponses AJAX | ⬜ Non applicable | Aucune requête AJAX en dehors des deux badges, dont l'un met son résultat en cache un jour. |
| `103` | Réduire au nécessaire les logs des serveurs | ⬜ Non applicable | Les journaux relèvent de la couche d'hébergement. |
| `104` | Désactiver le DNS lookup d’Apache | ⬜ Non applicable | Aucun Apache. |
| `105` | Apache Vhost : désactiver le AllowOverride | ⬜ Non applicable | Aucun Apache. |
...
| `4012` | Mettre en place une politique d'expiration et suppression des données | ⬜ Non applicable | Aucune donnée personnelle collectée, donc aucune politique d'expiration à écrire. |

## Utilisation

*15 règles : 7 respectées, 2 partielles, 6 non applicables.*
...
| `058` | Optimiser et générer les médias avant importation sur un CMS | ⬜ Non applicable | Aucun CMS. Les médias sont optimisés au build par `astro:assets`. |
| `060` | Encoder les sons en dehors du CMS | ⬜ Non applicable | Aucun contenu sonore. |
| `107` | Compresser les documents | ✅ Respectée | Aucun document téléchargeable n'est publié par le site. |
| `108` | Optimiser les PDF | ⬜ Non applicable | Aucun PDF publié. |
| `109` | Limiter les e-mails lourds et redondants | ⬜ Non applicable | Le site n'envoie aucun courriel. |
| `111` | Limiter la taille des e-mails envoyés | ⬜ Non applicable | Le site n'envoie aucun courriel. |
| `112` | Adapter les sons aux contextes d'écoute | ⬜ Non applicable | Aucun contenu sonore. |
...
*2 règles : 1 partielle, 1 non applicable.*
...
| `106` | Désactiver les logs binaires | ⬜ Non applicable | Aucun Apache. |
...
*4 règles : 1 respectée, 1 partielle, 2 non applicables.*
...
| `4032` | Mettre en place un "Circuit breaker" | ⬜ Non applicable | Aucun appel serveur à protéger. Les deux badges dégradent proprement si leur API ne répond pas. |
| `4033` | Favoriser le "Request collapsing" | ⬜ Non applicable | Aucune requête concurrente à regrouper. |
...
Les règles sont extraites des 119 fiches françaises du dépôt cnumr, avec leur étape de cycle de vie. Chaque verdict a été posé en relisant le code concerné et, quand la règle porte sur un comportement, en mesurant le build : `npm run check:eco` pour les poids et les requêtes, `npm run check:axe` pour l'accessibilité sur les 68 pages dans les deux thèmes, et Chrome piloté en CDP pour ce qui ne se lit pas dans le HTML.

**Quatre statuts.** *Respectée* : la règle est tenue et vérifiable. *Partielle* : tenue sur l'essentiel, avec un écart nommé. *Non applicable* : le site n'a pas l'objet de la règle, pas de base de données, pas de CMS, pas de courriel. *Non respectée* : l'écart est réel et corrigeable.

Une règle non applicable n'est pas un point gagné : ces trente-quatre lignes disent surtout ce que le site n'a pas, et c'est là que se joue l'essentiel de son empreinte.

## Autres référentiels

Le RGESN (référentiel général d'écoconception de services numériques, DINUM) couvre le même terrain avec 78 critères, mais il évalue aussi la gouvernance, la stratégie et l'hébergement, donc une partie porterait sur zatsit et non sur ce dépôt. Les [Web Sustainability Guidelines du W3C](https://w3c.github.io/sustyweb/) sont l'autre candidat sérieux, plus récentes et plus proches du métier. Dire lequel ajouter, et je le traite de la même façon.
