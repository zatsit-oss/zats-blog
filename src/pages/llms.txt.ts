...
export const GET: APIRoute = async ({ site }) => {
  ...
  // The rest of the estate. Without it an agent that lands here has no way out:
  // it reads a blog and never learns whose it is. The corporate site's own
  // llms.txt points back, so either file opens the whole estate.
  w('## Ailleurs');
  w();
  w(`- [zatsit](${WEBSITE_URL}) : l'entreprise, ses offres et ses bureaux, en français`);
  w(
    `- [zatsit sustainability](${SUSTAINABILITY_URL}) : nos outils de mesure d'impact, en anglais`,
  );
