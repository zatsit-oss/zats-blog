...
## The assumption this gate makes about production, and how it was wrong

Measuring text gzipped assumes the host compresses it. **That assumption held
for nobody, and this gate stayed green for the whole migration while it was
false.** Production is nginx on Cloud Run and `gzip on` was commented out in its
config, so the home page went over the wire at 72 kB where this script reported
21 kB. Every page weight the blog published described a site nobody received,
on the one site where an eco-design claim is meant to be checkable.

Compression was switched on 3 September 2026, so the figures are true again. Keep
the habit anyway, because the class of mistake is what matters: **a gate can
encode an assumption about production that nothing checks.** After any change to
the hosting layer, verify the response rather than the build:

```bash
curl -sI -H 'Accept-Encoding: gzip' https://blog.zatsit.fr/ | grep -i 'content-encoding\|cache-control'
curl -s -o /dev/null -w '%{size_download}\n' -H 'Accept-Encoding: gzip' https://blog.zatsit.fr/
```

Note that the nginx config is generated by Terraform, in the `zatsit-terraform`
repository, and is not editable from here.
